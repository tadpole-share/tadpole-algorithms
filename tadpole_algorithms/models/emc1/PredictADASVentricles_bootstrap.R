...
library(nlme)
library(splines)
X=read.table('intermediatedata.path');
IntermediateData = toString(X$V1[1])

mydata = read.csv(paste(IntermediateData,"/LongTADPOLE.csv",sep=""))

for (n_boot in 0:100)
{
  e=file.exists(paste(IntermediateData,"/PatientStages_",n_boot,".csv",sep=""))
  if (e==FALSE)
    {
     break 
    }
...
for (i in 0:(n_boot-1))
{
  str_out = paste(IntermediateData,"/TADPOLE_Submission_EMC1_",i,".csv",sep="")
  ds = read.csv(paste(IntermediateData,"/PatientStages_",i,".csv",sep=""),header=FALSE)
  dsorg = read.csv(paste(IntermediateData,"/PatientStages",".csv",sep=""),header=FALSE)
  mydata['DiseaseState']=ds
  mydata['DiseaseStateOrg']=dsorg
  #mydata['Rate']=ds$Rate
  lb2_subjects = read.csv(paste(IntermediateData,"/ToPredict.csv",sep=""),header=FALSE)
  ...
  model_ADAS<- lme(fixed=ADAS13~(ns(AGE,2) * ns(DiseaseState,4))+PTEDUCAT, random=~DiseaseState+DiseaseState^2|RID,data=data_ADAS, control = lmeControl(opt="optim",optimMethod="L-BFGS-B",maxIter=20,returnObject=TRUE))
  summary(model_ADAS)
  ...
  model_Ventricles<- lme(fixed=Ventricles~(ns(AGE,2) * ns(DiseaseState,4))+PTGENDER+ICV_bl, random=~DiseaseState+DiseaseState^2|RID ,data=data_Ventricles, control = lmeControl(opt='optim',optimMethod="L-BFGS-B",maxIter=20,returnObject=TRUE))
  summary(model_Ventricles)

  model_DiseaseState<- lme(fixed=DiseaseStateOrg~1, random=~AGE+AGE^2+AGE^3|RID ,data=mydata, control = lmeControl(opt='optim',optimMethod="L-BFGS-B",maxIter=20,returnObject=TRUE))
