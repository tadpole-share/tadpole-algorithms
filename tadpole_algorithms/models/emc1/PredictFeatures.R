...
str_out = paste(IntermediateData,"/PredictionMatrix.csv",sep="")

mydata = read.csv(paste(IntermediateData,"/LongTADPOLE.csv",sep=""))
#mydata <- mydata[complete.cases(mydata[,c('Diagnosis')]),]
ds = read.csv(paste(IntermediateData,"/PatientStages.csv",sep=""),header=FALSE)
...
h <- colnames(mydata)
idx_Feats = read.csv(paste(IntermediateData,"/FeatureIndices.csv",sep=""))
idx_Feats$x = idx_Feats$x+8

PredictionMatrix <- read.csv(paste(IntermediateData,"/PredictionMatrix.csv",sep=""))
...
write.csv(PredictionMatrix,paste(IntermediateData,"/PredictionMatrix.csv",sep=""),row.names = FALSE)
write.csv(mydata_pred,paste(IntermediateData,"/PredictedLongTADPOLE.csv",sep=""),row.names = FALSE)
