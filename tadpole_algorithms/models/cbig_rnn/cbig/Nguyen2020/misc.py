...
def censor_d1_table(_table):
    ...
    _table.drop(
        _table.loc[(_table.RID == 2190) & (_table.VISCODE == 'm03')].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 4579) & (_table.VISCODE == 'm03')].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 1088) & (_table.VISCODE == 'm72')].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 1195) & (_table.VISCODE == 'm48')].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 4960) & (_table.VISCODE == 'm48')].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 4674)].index, \
        inplace=True)
    _table.drop(
        _table.loc[(_table.RID == 5204)].index, \
        inplace=True)

    _table.reset_index()


def load_table(csv, columns, isTrain):
...
def get_mask(csv_path, use_validation, isTrain):
    """ Get masks from CSV file """
    columns = ['RID', 'EXAMDATE', 'VISCODE', 'train', 'val', 'test']
    frame = load_table(csv_path, columns, isTrain)
