...
def split_by_median_date(data, subjects):
    ...
    for rid in subjects:
        ...
        # Modification for Tadpole-Share
        median_date = np.sort(data.EXAMDATE[subj_mask])[subj_mask.sum() - 1]
        first_half[subj_mask & (data.EXAMDATE <= median_date)] = 1
        second_half[subj_mask & (data.EXAMDATE > median_date)] = 1
    return first_half, second_half


def gen_fold(data, nb_folds, outdir, name):
    """ Generate *nb_folds* cross-validation folds from *data """
    subjects = np.unique(data.RID)
    has_1tp = np.array([np.sum(data.RID == rid) >= 1 for rid in subjects])

    potential_targets = np.random.permutation(subjects[has_1tp])
    folds = np.array_split(potential_targets, nb_folds)

    leftover = [subjects[~has_1tp]]

    for test_fold in range(nb_folds):
        val_fold = (test_fold + 1) % nb_folds
        train_folds = [
            i for i in range(nb_folds)
        ]
        ...
        mask_frame = gen_mask_frame(data, train_timepoints, val_in_timepoints,
                                    test_in_timepoints)
        mask_frame.to_csv(
            path.join(outdir, name + 'fold%d_mask.csv' % test_fold), index=False)

        val_frame = gen_ref_frame(data, val_out_timepoints)
        val_frame.to_csv(
            path.join(outdir, name + 'fold%d_val.csv' % test_fold), index=False)

        test_frame = gen_ref_frame(data, test_out_timepoints)
        test_frame.to_csv(
            path.join(outdir, name + 'fold%d_test.csv' % test_fold), index=False)
...
def main(args, name):
    ...
    columns = ['RID', 'DXCHANGE', 'EXAMDATE', 'VISCODE']
    ...
    gen_fold(frame, args.folds, args.outdir, name)


if __name__ == '__main__':
    main(get_args(), name='train')
